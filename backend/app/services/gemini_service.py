...
class GeminiService:
    ...
    def __init__(self):
        # Use Gemini 2.5 Flash for faster responses and better structure
        ...
        self.generation_config = {
            "temperature": 0.15,
            "top_p": 0.9,
            "max_output_tokens": 8192,  # Increased to avoid MAX_TOKENS truncation
        }
...
Please provide a helpful, accurate response based on the context provided. Return a human-friendly response, and where possible return a short structured JSON snippet summarizing key fields (summary, earnings, deductions, comparisons, advice)."""
    ...
    async def generate_response_chunks(
        ...
        """
        Real-time streaming generator using Gemini's stream=True API.
        ...
        try:
            ...
            # VERBOSE: Print the full prompt being sent to AI
            # print(f"VERBOSE: Sending streaming prompt to AI (truncated):\n{prompt[:500]}...\n")  # Commented out for production
